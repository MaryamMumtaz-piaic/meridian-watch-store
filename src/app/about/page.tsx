        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="grid gap-16 lg:grid-cols-2 lg:gap-24">
            ...
            <div>
              ...
              <h2 className="mt-6 font-serif text-3xl font-bold leading-tight text-foreground sm:text-4xl">
              ...
              <div className="mt-8 space-y-5 text-sm leading-relaxed text-stone">
                ...
                <p>
                  ...
                  the pilot who reads altitude, the athlete who times laps, the professional who
                  values restraint, and the explorer who needs the case to survive what the day
                  cannot predict.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <blockquote className="border-l-4 border-gold pl-8 py-6">
                <p className="font-serif text-2xl font-medium leading-relaxed text-foreground">
                  &ldquo;A watch that tells you the time is a commodity.
                  A watch that holds the time — that is something else entirely.&rdquo;
                </p>
                <footer className="mt-5 text-xs font-medium uppercase tracking-[0.18em] text-stone">
                  — Founder, Maison Temps
                </footer>
              </blockquote>

              <div className="mt-4 grid grid-cols-2 gap-px border border-hairline bg-hairline">
                {[
                  { label: "Case tolerances", value: "±0.01 mm" },
                  { label: "Regulation target", value: "±3 sec/day" },
                  { label: "Movement components", value: "200+" },
                  { label: "Hours of testing", value: "72 hrs" },
                ].map((stat) => (
                  <div key={stat.label} className="bg-background p-6">
                    <p className="font-mono text-2xl font-semibold text-gold">{stat.value}</p>
                    <p className="mt-1 text-[11px] uppercase tracking-[0.14em] text-stone">
                      {stat.label}
                    </p>
                  </div>
                ))}
            ...
            {[
              {
                ...
                title: "Precision",
                body:
                  "Every component in a Maison Temps movement is machined to tolerances that defy the human eye. We hold ourselves to +/-0.01 millimetre across 200 individual parts — because a watch that loses ten seconds a day is, in our view, not yet finished.",
              },
              {
                icon: "◇",
                title: "Heritage",
                body:
                  "We build upon five decades of horological tradition, preserving hand-finishing techniques that modern production has largely abandoned. Our master watchmakers spend years in apprenticeship before touching a movement destined for a client's wrist.",
              },
              {
                icon: "◉",
                title: "Innovation",
                body:
                  "Tradition without progress is nostalgia. We invest in new case materials — Grade-5 titanium, anti-magnetic silicon escapements — while preserving the craft beneath them. The result is a watch prepared for the next century.",
              },
            ].map((v) => (
              <div key={v.title} className="group">
                <span className="text-3xl text-gold">{v.icon}</span>
                <h3 className="mt-5 font-serif text-xl font-bold text-foreground">{v.title}</h3>
                <div className="mt-3 h-px w-8 bg-gold transition-all duration-300 group-hover:w-16" />
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <div className="grid grid-cols-2 gap-px border border-white/10 bg-white/10 lg:grid-cols-4">
            {[
              ...
              <div
                key={stat.label}
                className="flex flex-col items-center justify-center bg-ink px-6 py-12 text-center"
              >
                <p className="font-serif text-5xl font-bold text-gold-bright sm:text-6xl">
                  {stat.number}
                </p>
                <p className="mt-3 text-xs font-medium uppercase tracking-[0.18em] text-parchment/60">
                  {stat.label}
                </p>
              </div>
            ))}
      ...
      <section className="py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="flex flex-col items-center gap-8 text-center">
            ...
            <Link
              href="/craftsmanship"
              className="group inline-flex items-center gap-3 border border-foreground bg-foreground px-8 py-3.5 text-xs font-semibold uppercase tracking-[0.14em] text-white transition-all duration-300 hover:border-gold hover:bg-gold"
            >
